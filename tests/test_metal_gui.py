...
def test_a_map_rebuilt_from_a_db_renders_the_same(app, metal, tmp_path):
    ...
    The db here is the harder of the two flavours: the sub-block's was written by a run that
    never saw this design - dumped on its own, so its grids belong to another die - and the map
    on screen had to be re-rasterised from the shapes it stored before the window could draw it.
    Both flavours land in the same window through the same code, so one arm is the whole claim;
    the data-level equivalence of the *other* one - a db that answers with its grids - is
    `tests/test_metal_db.py`'s, which compares the capacity, the cell detail and the warnings too.
    """
    import contextlib
    import io
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        build_metal([f"{SAMPLE}/sub.def"], [f"{SAMPLE}/cells.lef"], [f"{SAMPLE}/tech.lef"],
                    grid_size=10.0, dump_db=str(alone))
        replayed = build_metal([f"{SAMPLE}/top.def"], [f"{SAMPLE}/cells.lef"],
                               [f"{SAMPLE}/tech.lef"], grid_size=10.0,
                               db_paths=[str(alone / "sub.def.db")])
    ...
    try:
        ...
        assert list(parsed_win._panel._boxes) == list(replayed_win._panel._boxes)
        for layer, boxes in parsed_win._panel._boxes.items():
            assert boxes[0].text() == replayed_win._panel._boxes[layer][0].text(), layer
        metal.set_scope("all")
        replayed.set_scope("all")
        kind = metal.kinds()[0][0]
        assert np.array_equal(np.asarray(metal.heat(kind)), np.asarray(replayed.heat(kind)))
        assert parsed_win._panel.peak_label.text() == replayed_win._panel.peak_label.text()
    finally:
        parsed_win.close()
        replayed_win.close()
