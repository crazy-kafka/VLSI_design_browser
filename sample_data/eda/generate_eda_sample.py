...
PIN_HALF = 0.05         # signal pins are 0.1 x 0.1 um
RAIL_HEIGHT = 0.1       # the rails span the cell's full width


def signal_pins(name):
    """The signal pin names a cell of this kind would declare, in macro-local microns."""
    if name.startswith("SRAM"):
        return ["CLK", "WE", "RE", "ADDR0", "ADDR1", "DIN0", "DIN1", "DOUT0", "DOUT1"]
    if name.startswith(("SDF", "MB4_SDF")):
        return ["D", "SI", "SE", "CK", "Q"]
    if name.startswith("ICG"):
        return ["CK", "E", "GCK"]
    if name.startswith("MUX2"):
        return ["A", "B", "S", "Z"]
    if name.startswith(("NAND2", "NOR2", "AND2", "OR2", "XOR2")):
        return ["A", "B", "ZN"]
    return ["A", "ZN"]      # INV_X* / BUF_X* / CLKBUF_X*


def pin_lines(name, width, height, physical):
    """The ``PIN`` stanzas for one macro: its signal pins, then the two rails.

    Every real LEF declares pins, and without them the physical mode's pin-density map has
    nothing to count. The rails are here to be *left out* of it - by ``USE``, and by name for
    the libraries that write ``USE SIGNAL`` on a VDD pin - and a physical-only cell declares
    no signal pin at all, so it contributes nothing to that map either way.
    """
    lines = []
    pins = [] if physical else signal_pins(name)
    step = width / (len(pins) + 1)
    for index, pin in enumerate(pins):
        # Inputs along the bottom edge, the output along the top, spread across the cell.
        output = index == len(pins) - 1
        cx = step * (index + 1)
        cy = height - 0.15 if output else 0.15
        lines += [f"  PIN {pin}",
                  f"    DIRECTION {'OUTPUT' if output else 'INPUT'} ;",
                  "    USE SIGNAL ;",
                  "    PORT",
                  "      LAYER metal1 ;",
                  f"      RECT {cx - PIN_HALF:.3f} {cy - PIN_HALF:.3f} "
                  f"{cx + PIN_HALF:.3f} {cy + PIN_HALF:.3f} ;",
                  "    END",
                  f"  END {pin}"]
    for pin, use, (y0, y1) in (("VDD", "POWER", (height - RAIL_HEIGHT, height)),
                               ("VSS", "GROUND", (0.0, RAIL_HEIGHT))):
        lines += [f"  PIN {pin}",
                  "    DIRECTION INOUT ;",
                  f"    USE {use} ;",
                  "    PORT",
...
def write_lef(path):
    ...
    for name, width, height, klass, _weight, physical in CELLS:
        lines += [f"MACRO {name}",
                  f"  CLASS {klass} ;",
                  "  ORIGIN 0 0 ;",
                  f"  SIZE {width} BY {height} ;",
                  "  SYMMETRY X Y ;"]
        lines += pin_lines(name, width, height, physical)
        lines += [f"END {name}", ""]
...
def verify():
    ...
    # Every signal pin is counted once and no rail is: the filter reads both USE and name.
    for name in cells:
        expected = 0 if name in AREA_ONLY else len(signal_pins(name))
        assert len(pins.get(name, ())) == expected, (name, pins.get(name))
    counted = sum(len(v) for v in pins.values())
    print(f"  verify: {len(cells)} LEF macro(s); {counted} signal pin(s) for the pin-density "
          f"map; physical-only {' '.join(sorted(AREA_ONLY))}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # The pin map is the def flow's only extra, and it counts the same placements again.
        expected = sum(len(pins.get(i["cell_name"], ()))
                       for i in block["instances"].values())
        pin_grid = build_physical([bpath], cpath, grid_size=2.0, pins=pins).heat("pins")
