...
**Browse a netlist's hierarchy and its layout together** — a PyQt5 desktop app for
physical-design engineers: a sortable, searchable tree of per-hierarchy metrics on one side, the
placed design or its routing on the other.

<p>
  <a href="#usage"><img src="docs/compare.png" width="32%" alt="Two-version compare: V1 / V2 / Diff tabs above a hierarchy table of area, count, ULVT%, MB%, D1D2% and cell-type counts, each metric cell carrying a data bar or a red-to-green quality gradient."></a>
  <a href="#physical-layout-mode"><img src="docs/physical.png" width="32%" alt="Physical mode: the hierarchy tree beside a 2-D cell-density heat map of a CPU cluster, with a thermal legend and a Density% column."></a>
  <a href="#metal-density-mode"><img src="docs/metal.png" width="32%" alt="Metal mode: a per-routing-layer utilisation map, with the hovered cell's arithmetic on the left and the layer table (width, spacing, pitch, direction, utilisation) on the right."></a>
</p>

*Captures of the running app, regenerated by `python docs/capture.py`.*
...
python quickstart.py json       # two-version compare over sample_data/*.json
python quickstart.py physical   # a ~100k-instance CPU cluster as a density heat map
python quickstart.py metal      # per-layer routing utilisation of a routed DEF
```

Nothing is written to disk. `python quickstart.py` lists all five shortcuts — `def` and `verilog`
read EDA files directly — and extra flags pass through to the real CLI
(`python quickstart.py def --grid_size 2.0`).

## What it does

| mode | reads | shows |
|---|---|---|
| `json` | `cell_info.json` + `instance_info.json` | the hierarchy table, or **V1 / V2 / Δ** two-version tabs |
| `def` | DEF + macro LEF | the same, built from EDA files; placement comes from the DEF, so **physical mode** works |
| `verilog` | gate-level netlist + macro LEF | the hierarchy only — a netlist carries no placement |
| `metal` | routed DEF + macro and tech LEFs | a **per-routing-layer utilisation map** of the wiring |

LEF, DEF, Verilog and JSON are all accepted (`.gz` read transparently), and the EDA flows convert
to the JSON form first, so one pipeline serves the first three. What each format contributes is
under [EDA input formats](#eda-input-formats); if you want the parsers themselves, their
interfaces are in [PARSERS.md](PARSERS.md).

It has been run on real designs, and the numbers are on record rather than asserted:

- **3,355,697 placed instances, 17,385 macros from 221 LEF files, 18 routing layers** in a
  ~30 M-line routed DEF — [`real_design_case_record.md`](dev_plan/real_design_case_record.md)
- that design's map is a **1,785.87 s** read, 1,696.70 s of it the routing parse (**112.9 M** wire
  shapes); parsed once into a db, a rebuild is **~23 s** —
  [`metal_def_db.md`](dev_plan/metal_def_db.md)
- where a rebuild cannot apply, stored shapes replay at **0.50 µs each** against 19.45 µs to parse
  them — [`metal_def_db_eval.md`](dev_plan/metal_def_db_eval.md)

## Features

**Find the hierarchy you want**
...
- **Search** — exact / wildcard / regex, case-insensitive; results in a popup
  window that jumps the tree to the selected hierarchy, and whose columns sort by
  value when you click a header (a missing value sorts first). In compare mode the
  popup is split into V1/V2 panels and clicking a row jumps to that version's tab.
...
[Requirements & install](#requirements--install) · [Quick start](#quick-start) · [Usage](#usage) ·
[Input format](#input-format) · [EDA input formats](#eda-input-formats) · [Metrics](#metrics) ·
[Physical layout mode](#physical-layout-mode) · [Hierarchy contours & density](#hierarchy-contours--density) ·
[Metal density mode](#metal-density-mode) · [Architecture](#architecture) · [Testing](#testing)
...
![Physical mode on the bundled CPU-cluster sample: the hierarchy tree on the left, the cell-density heat map on the right.](docs/physical.png)
...
![Metal mode on the bundled sample: the map of a 1P12M stack with the layer table on the right.](docs/metal.png)
...
[`sample_data/real/PROVENANCE.md`](sample_data/real/PROVENANCE.md), and the reasoning — including
which files have to come out before this repository is published — in
[`dev_plan/real_sample_sources.md`](dev_plan/real_sample_sources.md).
